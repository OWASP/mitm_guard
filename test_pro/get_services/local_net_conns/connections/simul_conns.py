import ipaddress
class simul_conns:
    def __init__(self, result_estab, result_listen, internal_subnets, cont_all_local):
        self.simultaneously_conns = self.simultaneously_listen_estab(result_estab, result_listen, internal_subnets, cont_all_local)
    def get_class_name(self):
        return self.__class__.__name__
    def simultaneously_listen_estab(self, result_estab, result_listen, internal_subnets, cont_all_local):
          all_dict_simul = {}
          cont_simul = {}
          all_listen = {}
          isimult = 0
          ...
          for in_result in range(len(result_estab)):
              for in_listen in range(len(result_listen)):
                  if (result_estab[in_result+1]['l_ip'] and result_estab[in_result+1]['l_p']) == \
                     ...
                     all_listen = {}
          cont_all_local.setdefault(self.get_class_name(),cont_simul)
          return cont_simul
