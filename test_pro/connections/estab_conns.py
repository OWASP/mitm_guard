from ..get_services import ipv4_valid
from ..get_services import all_ps
from ..get_services import save_in_file
import ipaddress
class estab_conns:
    def __init__(self, conns,internal_subnets,cont_all):
        self.estab_conn = self.estabilished_conns(conns,internal_subnets,cont_all)
    def get_class_name(self):
        return __class__.__name__
    def estabilished_conns(self,conns,internal_subnets,cont_all):
        all_dict_estab = {}
        all_estab = {}
        list_all_dict_estab = {}
        iestab = 0
        ...
        for in_conns_estab in range(len(conns)):
            validate = ipv4_valid.ipv4_valid(conns[in_conns_estab][3][0])
            if conns[in_conns_estab][5] == "ESTABLISHED" and validate.ip_val and \
               conns[in_conns_estab][3][0] == conns[in_conns_estab][4][0]:
                ...
                all_estab = {}
        save_in_file.save_in_file(cont_all,self.get_class_name(),list_all_dict_estab)
