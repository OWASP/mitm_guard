...
class listen_conns:
    def __init__(self, conns, internal_subnets, cont_all):
        self.lis_conn = self.lis_conns(conns,internal_subnets,cont_all)
    def get_class_name(self):
        return self.__class__.__name__
    def lis_conns(self,conns,internal_subnets,cont_all):
        all_listen = {}
        all_dict_listen = {}
        ...
        for in_conns in range(len(conns)):
            validate = ipv4_valid.ipv4_valid(conns[in_conns][3][0])
            if (conns[in_conns][5] == "LISTEN") and ((conns[in_conns][3][0] == '::1') or (conns[in_conns][3][0] == '127.0.0.1')):
                ...
                all_listen = {}
        save_in_file.save_in_file(cont_all,self.get_class_name(),list_all_dict_listen)
