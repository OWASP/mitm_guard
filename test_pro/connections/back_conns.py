...
class back_conns:
    def __init__(self, result_estab, result_listen, cont_all):
        self.b_conns = self.backreverse_conns(result_estab, result_listen, cont_all)
    def get_class_name(self):
        return self.__class__.__name__
    def backreverse_conns(self, result_estab, result_listen, cont_all):
        all_dict_back = {}
        cont_back = {}
        list_ports_listen = []
        list_ports_estab = []
        ...
        for len_result_list_ports_back in range(len(result_list_ports_back)):
            for in_result in range(len(result_estab)):
                if result_list_ports_back[len_result_list_ports_back] == result_estab[in_result+1]['l_p']:
                   ...
                   all_conn_back = {}
        save_in_file.save_in_file(cont_all,self.get_class_name(),cont_back)
        return cont_back
