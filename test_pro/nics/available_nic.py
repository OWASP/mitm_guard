...
import hashlib
class available_nic:
    def __init__(self, status_nics, all_nics, cont_all):
        self.available_n = self.avail_nic(status_nics, all_nics, cont_all)
    def get_class_name(self):
        return __class__.__name__
    def avail_nic(self, status_nics, all_nics, cont_all):
        list_all_key = []
        list_all_val = []
        list_new_nics = []
        dict_available_nics = {}
        ...
        for m in range(len(status_nics)):
            if (list_all_val[m] == 'down') or ((list_all_val[m] == 'up') and \
               (netifaces.ifaddresses(all_nics[m])[netifaces.AF_INET][0]['addr'] == "127.0.0.1")):
                ...
                ii = ii+1
        save_in_file.save_in_file(cont_all,self.get_class_name(),dict_available_nics)
